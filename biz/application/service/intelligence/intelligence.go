...
var IntelligenceSVC *IntelligenceService
...
func (i *IntelligenceService) ListIntelligence(ctx context.Context, req *core_api.ListIntelligenceReq) (*core_api.ListIntelligenceResp, error) {
	header := http.Header{}
	header.Set("Cookie", conf.GetConfig().Coze.GetCookie(ctx))
	...
	if resp["code"].(float64) != 0 && resp["code"].(float64) != 700012006 {
		return nil, errorx.New(errno.ErrListIntelligence, errorx.KV("message", resp["message"].(string)))
	}
	if resp["code"].(float64) == 700012006 {
		header.Set("Cookie", conf.GetConfig().Coze.RefreshCookie(ctx))
		resp, err = httpx.GetHttpClient().Post(ctx, url, header, listBody)
		if err != nil {
			return nil, errorx.WrapByCode(err, errno.SynapseErrCode, errorx.KV("url", url))
		}
...
func (i *IntelligenceService) GetIntelligenceInfo(ctx context.Context, req *core_api.GetIntelligenceReq) (*core_api.GetIntelligenceResp, error) {
	header := http.Header{}
	header.Set("Authorization", "Bearer "+conf.GetConfig().Coze.PAT)
	header.Set("Content-Type", "application/json")
	...
	if resp["code"].(float64) != 0 {
		return nil, errorx.New(errno.ErrGetIntelligence, errorx.KV("message", resp["message"].(string)))
	}
	data := resp["data"].(map[string]interface{})
